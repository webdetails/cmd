...
function iScroll (el, options) {
	...
	if (that.options.HWCompositing) {
		...
		that.scroller.style.cssText += '-moz-transition-property:-moz-transform;-moz-transform-origin:0 0;-moz-transform:' + trnOpen + '0,0' + trnClose;
	} else {
		that.scroller.style.cssText += '-webkit-transition-property:top,left;-webkit-transform-origin:0 0;top:0;left:0';
		that.scroller.style.cssText += '-moz-transition-property:top,left;-moz-transform-origin:0 0;top:0;left:0';
	}
	...
	if (that.options.HWTransition) {
		...
		that.scroller.style.cssText += '-moz-transition-timing-function:cubic-bezier(0.33,0.66,0.66,1);-moz-transition-duration:0;';
	}
...
iScroll.prototype = {
	...
	_scrollbar: function (dir) {
		...
		if (!that[dir + 'ScrollbarWrapper']) {
			...
			bar = doc.createElement('div');
			if (!that.options.scrollbarClass) {
				bar.style.cssText = 'position:absolute;z-index:100;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.9);-webkit-background-clip:padding-box;-webkit-box-sizing:border-box;-moz-box-sizing:border-box;' + (dir == 'h' ? 'height:100%;-webkit-border-radius:4px 3px;' : 'width:100%;-webkit-border-radius:3px 4px;-moz-border-radius:3px 4px;');
			}
			bar.style.cssText += 'pointer-events:none;';
			bar.style.cssText += '-webkit-transition-property:-webkit-transform;-webkit-transition-timing-function:cubic-bezier(0.33,0.66,0.66,1);-webkit-transition-duration:0;-webkit-transform:' + trnOpen + '0,0' + trnClose;
			bar.style.cssText += '-moz-transition-property:-moz-transform;-moz-transition-timing-function:cubic-bezier(0.33,0.66,0.66,1);-moz-transition-duration:0;-moz-transform:' + trnOpen + '0,0' + trnClose;
